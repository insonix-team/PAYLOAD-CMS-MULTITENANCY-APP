                ...
                <div className="w-full h-[424px] overflow-hidden shadow-lg rounded-xs">
                  <Link href={card.cta?.link || `/services/${card.slug}`} className="group relative block w-full h-full">
                    ...
                    <div className="relative h-full flex flex-col justify-end px-4 text-white pb-5">
                      <h3 className="inline-flex w-fit my-4 px-4 py-1.5 rounded-full bg-secondary text-secondary-500 text-sm font-semibold tracking-wide shadow-sm group-hover:bg-primary transition">{card.title}</h3>

                      <p className="text-left text-sm line-clamp-3">{card.description}</p>

                      {/* ⭐ CTA BUTTON */}
                      {card.cta?.text && <span className="inline-block bg-primary/60 px-5 py-2 mt-3 text-sm font-semibold uppercase transition-all duration-300 opacity-80 group-hover:opacity-100 relative overflow-hidden after:absolute after:left-0 after:bottom-0 after:h-[3px] after:w-0 after:bg-secondary-500 after:transition-all after:duration-300 group-hover:after:w-full">{card.cta.text} →</span>}
                    </div>
                  </Link>
                </div>
              </motion.div>
