...
export const ContentImageComponentUI = ({ data }: Props) => {
  ...
  return (
    <section className={`  w-full py-12 ${bgClass} ${paddingClass}`}>
      <div
        className={`container mx-auto flex flex-col items-center gap-10 md:flex-row ${!isLeft ? 'md:flex-row-reverse' : ''}`}
      >
        {/* IMAGE BLOCK */}
          ...
          >
            ...
            <div className="relative w-full overflow-hidden rounded-xl shadow-md">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-6 py-5">
                {relatedText && (
                  <p className="font-serif text-xl font-light italic text-secondary-50">
                    {relatedText}
                  </p>
                )}
            ...
            {mediaUrl && (
              ...
              >
                ...
                <div className="absolute bottom-0 p-3">
                  {imageShortDescription && (
                    <p className="hidden text-[8px] font-semibold uppercase leading-4 tracking-[0.18em] text-secondary-50 md:block">
                      {imageShortDescription}
                    </p>
                  )}
              ...
              >
                {relatedText && (
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-semibold uppercase tracking-[0.2em] text-green-700">
                      {relatedText}
                    </span>
                  </div>
                )}
                ...
                {availabilityText && (
                  <p className="font-serif text-lg text-primary">{availabilityText}</p>
                )}
              </div>
            )}
          </motion.div>
