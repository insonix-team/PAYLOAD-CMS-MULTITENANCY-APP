    ...
    <header className={['fixed w-full z-[999] top-0 bg-white backdrop-blur-xl transition-all duration-300 ', small ? 'shadow-md' : 'shadow-sm'].join(' ')}>
      <div className="mx-auto  flex items-center justify-between h-20">
        ...
        <div className="hidden xl:block flex justify-end w-full xl:w-auto p-0  gap-3">
          {ctas?.map((cta: any, i: number) => {
            ...
            return (
              <Link
                key={ctaKey}
                href={href}
                className={`bg-primary uppercase text-white! border-0 hover:bg-primary-600 shadow-2xl cursor-pointer px-8 md:px-8! py-2 transition  block rounded-0 font-medium  whitespace-nowrap ${variants[cta.style || 'primary']}`}
              >
                {cta.label}
              </Link>
            );
          ...
          <Link
            href={'#'}
            className={`uppercase  border-0 hover:bg-primary-600 shadow-2xl cursor-pointer px-8 md:px-8! py-2  flex justify-center gap-1 text-white! font-medium transition bg-secondary whitespace-nowrap }`}
          >
