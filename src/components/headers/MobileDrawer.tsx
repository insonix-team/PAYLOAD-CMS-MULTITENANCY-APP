      ...
      >
        ...
        <div className="p-4 space-y-4 overflow-y-auto h-[calc(100vh-64px)]">
          ...
          {ctas?.map((cta: any, i: number) => {
            ...
            return (
              <Link
                key={cta?.id ?? `${cta.label}-${i}`}
                href={href}
                onClick={() => setOpen(false)}
                className={`uppercase  border-0 hover:bg-primary-600 shadow-2xl cursor-pointer px-8 md:px-8! py-2 block text-center w-full  font-semibold mt-2 ${style[cta.style]}`}
              >
                {cta.label}
              </Link>
            );
