...
export const HeroLeftLayoutUI = ({ data }: Props) => {
  const { backgroundImage, overlayOpacity = 50, tag, title, subtitle, description, buttons = [] } = data

  const bgUrl = typeof backgroundImage === 'string' ? backgroundImage : (backgroundImage?.url ?? '')

  const opacity = (overlayOpacity ?? 50) / 100
  const hasButtons = buttons.length > 0

  return (
    <section className="relative flex items-center justify-start text-white min-h-[90vh] bg-cover bg-top" style={{ backgroundImage: `url(${bgUrl})` }}>
      <div className="absolute inset-0 bg-black" style={{ opacity }} />

      <div className="relative z-10 max-w-4xl text-left px-6">
        {tag && <span className="inline-block mb-3 px-3 py-1 text-sm bg-white/20 rounded-full">{tag}</span>}
